...
def compute_bm25(query_terms, descriptor_file, inverse_file, k=1.5, b=0.75):
    """
    ...
    """
    # Load data
    descriptor_df = pd.read_csv(descriptor_file, sep=r"\s+", names=["doc_id", "term", "freq", "weight"], engine="python")
    inverse_df = pd.read_csv(inverse_file, sep=r"\s+", names=["term", "doc_id", "freq", "weight"], engine="python")

    # Precompute document lengths and average document length
    doc_lengths = descriptor_df.groupby("doc_id")["freq"].sum()
    avg_doc_len = doc_lengths.mean()
    N = descriptor_df["doc_id"].nunique()
    ...
    for term in query_terms:
        ...
        term_data = inverse_df[inverse_df["term"] == term]
