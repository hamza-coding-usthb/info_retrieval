...
def cosine_similarity(query_terms, query_weights, inverse_df):
    """
    ...
    unique_docs = inverse_df["doc_id"].unique()
    unique_terms = inverse_df["term"].unique()
...
def cosine_similarity_2(query_terms, query_weights, inverse_df):
    """
    ...
    for term in query_terms:
        ...
        if term in query_weights and term in inverse_df["term"].values:
            ...
            term_data = inverse_df[inverse_df["term"] == term].copy()
...
def weighted_jaccard_index(query_weights, descriptor_df):
    """
    ...
    for doc_id, doc_data in descriptor_df.groupby("doc_id"):
...
def compute_rsv(query, descriptor_file, inverse_file, method):
    """
    ...
    """
    descriptor_df = load_data(descriptor_file, use_index=False)
    inverse_df = load_data(inverse_file, use_index=True)
    query_weights = compute_query_weights(query)

    if method == "Scalar Product":
        return scalar_product(query, query_weights, inverse_df)
    elif method == "Cosine Similarity":
        return cosine_similarity(query, query_weights, inverse_df)
    elif method == "Weighted Jaccard Index":
        return weighted_jaccard_index(query_weights, descriptor_df)
    else:
