...
class SearchInterface(QMainWindow):
    ...
    def perform_search(self):
        ...
        try:
            # Load the dataset only once for efficiency
            descriptor_file = "DescriptorSplit.txt"
            df = pd.read_csv(descriptor_file, delim_whitespace=True, header=None, names=["doc_id", "term", "freq", "weight"])
            term_to_docs = df.groupby("term")["doc_id"].apply(set).to_dict()

            if self.boolean_checkbox.isChecked():
                # Validate and evaluate the Boolean query
                if not validate_boolean_query(query):
                    QMessageBox.critical(self, "Error", "Invalid Boolean query.")
                    return

                results = evaluate_boolean_query_with_pandas(query, term_to_docs)
                ...
                bm25_scores = compute_bm25(query.split(), descriptor_file, None, k, b)
                self.display_rsv_results(bm25_scores, "BM25")

            elif self.rsv_checkbox.isChecked():
                # RSV search
                rsv_method = self.rsv_method_dropdown.currentText()
                rsv_scores = compute_rsv(query.split(), descriptor_file, None, rsv_method)
                ...
                token_method = "Split" if self.split_rb.isChecked() else "Token"
                ...
                if self.no_stem_rb.isChecked():
                    ...
                    stem_method = "Lancaster"
                else:
                    stem_method = ""

                results = load_descriptor_or_index(query, search_type, token_method, stem_method, use_index=(search_type == "docs_per_term"))
